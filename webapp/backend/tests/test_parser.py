...
class ParserTests(unittest.TestCase):
    ...
    def test_parser_supports_new_v10_markers(self) -> None:
        content = (
    ...
    def test_parser_supports_v10_alias_markers(self) -> None:
        content = (
    ...
    def test_parser_supports_body_caption_and_lot_lof_aliases(self) -> None:
        content = (
            "TOC:\n"
            "LOT:\n"
            "LOF:\n"
        ...
        self.assertIn("list_of_figures", types)
        self.assertIn("table_caption", types)
        self.assertIn("figure_caption", types)

        paragraph = next(node for node in result.nodes if node.type == "paragraph")
        self.assertEqual(paragraph.text, "first line\nsecond line")

    def test_parser_keeps_sticky_heading_caption_and_reference_blocks(self) -> None:
        content = (
            "H1: Release Notes\n"
            "Quarterly Update\n"
            "H2: Overview\n"
            ...
            "TABLE: Metric | Value\n"
            "TABLE: Open Bugs | 4\n"
            "CAPTION: Release metrics\n"
            "Captured from weekly review\n"
            "REFERENCES:\n"
            "REFERENCE: [1] Internal release board\n"
            "Reviewed on 2026-04-05\n"
        )
        result = parse_notesforge(content)

        headings = [n for n in result.nodes if n.type == "heading"]
        self.assertEqual(headings[0].text, "Release Notes\nQuarterly Update")
        self.assertEqual(headings[1].text, "Overview\nMore heading detail")

        captions = [n for n in result.nodes if n.type == "table_caption"]
        self.assertEqual(len(captions), 1)
        self.assertEqual(captions[0].text, "Release metrics\nCaptured from weekly review")

        references = [n for n in result.nodes if n.type == "reference"]
        self.assertEqual(len(references), 1)
        self.assertEqual(references[0].text, "[1] Internal release board\nReviewed on 2026-04-05")

    def test_markdown_conversion_generates_toc_lot_lof_and_hierarchical_numbering(self) -> None:
        content = (
            "TOC:\n"
            "LOT:\n"
            "LOF:\n"
        ...
        self.assertIn("## List of Tables", md)
        self.assertIn("## List of Figures", md)
        self.assertIn("*Table 1: Demo table*", md)
        self.assertIn("*Figure 1: Demo figure*", md)
        self.assertIn("1. Parent", md)
        self.assertIn("1.1. Child", md)
