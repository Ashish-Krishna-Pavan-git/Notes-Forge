...
from .themes import css_from_theme
...
def _runtime_target() -> str:
    return (os.environ.get("NF_RUNTIME_TARGET") or os.environ.get("NF_APP_RUNTIME") or "").strip().lower()


def _prefer_ilovepdf_first() -> bool:
    raw = (os.environ.get("NF_PDF_PROVIDER") or os.environ.get("NF_PDF_PROVIDER_PREFERENCE") or "").strip().lower()
    if raw in {"ilovepdf", "ilovepdf_first", "external", "external_first"}:
        return True
    if raw in {"local", "local_first"}:
        return False
    has_external = bool(
    ...
    if target == "web":
        return has_external
    if target in {"desktop", "windows", "linux", "macos"} or target.startswith("desktop_"):
        return False
    return has_external
...
def _max_image_bytes() -> int:
    raw = os.environ.get("NF_MAX_IMAGE_BYTES")
    try:
        value = int(raw) if raw is not None else 8 * 1024 * 1024
    except ValueError:
        value = 8 * 1024 * 1024
    return max(128 * 1024, min(25 * 1024 * 1024, value))
...
def _collect_toc_entries(nodes: Sequence[Node]) -> list[tuple[int, str]]:
    ...
    for node in nodes:
        if node.type == "heading":
            ...
            if title:
                entries.append((max(1, min(6, node.level)), title))
            continue
        ...
        if node.type == "chapter":
            chapter_idx += 1
            entries.append((1, f"Chapter {chapter_idx}: {(node.text or 'Chapter').strip()}"))
            continue
        if node.type == "appendix":
            entries.append((1, f"Appendix: {(node.text or 'Appendix').strip()}"))
            continue
        if node.type == "references_heading":
            title = (node.text or "References").strip()
            if title:
                entries.append((1, title))
    return entries


def _numbered_labels(levels: Sequence[int], count: int) -> list[str]:
    labels: list[str] = []
    counters: list[int] = []
    for idx in range(count):
        raw_level = levels[idx] if idx < len(levels) else 0
        level = max(0, min(raw_level, len(counters)))
        while len(counters) <= level:
            counters.append(0)
        counters[level] += 1
        counters = counters[: level + 1]
        labels.append(".".join(str(item) for item in counters))
    return labels
...
def _image_bytes_within_limit(payload: bytes, warnings: List[str], source_label: str) -> bool:
    limit = _max_image_bytes()
    if len(payload) > limit:
        warnings.append(
            f"Image '{source_label}' exceeds the {limit // (1024 * 1024)} MB safety limit and was skipped."
        )
        return False
    return True


def _read_remote_image_bytes(value: str, warnings: List[str]) -> bytes | None:
    request = Request(
        value,
        method="GET",
        headers={"User-Agent": "notesforge/10.0.0"},
    )
    with urlopen(request, timeout=12) as response:  # nosec B310
        ...
        if content_length:
            ...
            if announced > limit:
                warnings.append(
                    f"Image '{mask_sensitive_url(value)}' exceeds the {limit // (1024 * 1024)} MB safety limit and was skipped."
                )
                return None
...
def _resolve_image_stream(source: str, warnings: List[str]) -> io.BytesIO | None:
    ...
    try:
        ...
        if decoded:
            if not _image_bytes_within_limit(decoded, warnings, "data-uri image"):
                return None
        ...
        if value.lower().startswith(("http://", "https://")):
            ...
            if not ok:
                ...
                return None
            payload = _read_remote_image_bytes(value, warnings)
            return io.BytesIO(payload) if payload else None

        local = Path(value)
        if local.exists() and local.is_file():
            if local.suffix.lower() not in ALLOWED_IMAGE_SUFFIXES:
                warnings.append(f"Blocked local image '{local.name}': unsupported file type.")
                return None
            payload = local.read_bytes()
            if not _image_bytes_within_limit(payload, warnings, local.name):
                return None
...
def _apply_page_borders(
    ...
    *,
    ...
    pg_borders.set(qn("w:shadow"), "true" if shadow else "false")
    pg_borders.set(qn("w:frame"), "true" if frame else "false")
    color_hex = (color or "").strip().lstrip("#")
    if len(color_hex) != 6:
        color_hex = "000000"
    sz = max(2, min(96, int(round(max(0.25, width_pt) * 8))))
...
def _preview_page_number_text(mode: str, format_template: str) -> str:
    return _normalize_page_template(mode, format_template)


def _resolved_page_number_text(current_page: int, total_pages: int, mode: str, format_template: str) -> str:
    text = _normalize_page_template(mode, format_template)
    text = re.sub(r"X", str(current_page), text, flags=re.IGNORECASE)
    text = re.sub(r"Y", str(total_pages), text, flags=re.IGNORECASE)
    return text
...
def _inject_preview_running_blocks(
    ...
    mode = security.pageNumberMode if security.pageNumberMode in {"page_x", "page_x_of_y"} else "page_x"
    ...
    page_number_text = _preview_page_number_text(
        mode,
        _style_str(
            styles,
            "page_number_format",
            "pageNumberFormat",
            default="Page X of Y" if mode == "page_x_of_y" else "Page X",
        ),
    )
    ...
    if header_text:
        header_parts.append(f'<span class="nf-header-text">{escape(header_text)}</span>')
    if footer_text:
        footer_parts.append(f'<span class="nf-footer-text">{escape(footer_text)}</span>')
    if page_number_position == "header" and show_header_page_numbers:
        header_parts.append(f'<span class="nf-page-num-preview">{escape(page_number_text)}</span>')
    if page_number_position == "footer" and show_footer_page_numbers:
        footer_parts.append(f'<span class="nf-page-num-preview">{escape(page_number_text)}</span>')
    if header_parts:
        injected.append(f'<div class="nf-running-header">{" ".join(header_parts)}</div>')
    if footer_parts:
...
def _http_json(
    ...
    *,
    ...
    headers = {"Accept": "application/json", "User-Agent": "notesforge/10.0.0"}
    body = None
    if token:
...
def _nodes_to_plain_lines(nodes: Sequence[Node]) -> list[str]:
    ...
    toc_entries = _collect_toc_entries(nodes)
    for node in nodes:
        if node.type == "heading":
            ...
            lines.append((node.text or "Table of Contents").strip())
            for level, item in toc_entries or [(1, "TOC entries appear as headings are added.")]:
                lines.append(f"{'  ' * max(0, level - 1)}- {item}")
            lines.append("")
        elif node.type == "list_of_tables":
            lines.append((node.text or "List of Tables").strip())
            for entry in table_entries or ["Table entries are generated when captions are available."]:
                lines.append(f"- {entry}")
            lines.append("")
        elif node.type == "list_of_figures":
            lines.append((node.text or "List of Figures").strip())
            ...
            for idx, item in enumerate(node.items or []):
                ...
                lines.append(f"{'  ' * item_level}- {item}".rstrip())
            lines.append("")
        elif node.type == "numbered":
            levels = node.levels or []
            labels = _numbered_labels(levels, len(node.items or []))
            for idx, item in enumerate(node.items or []):
                item_level = levels[idx] if idx < len(levels) else 0
                item_label = labels[idx] if idx < len(labels) else str(idx + 1)
                lines.append(f"{'  ' * item_level}{item_label}. {item}".rstrip())
            lines.append("")
            ...
            for idx, item in enumerate(node.items or []):
                ...
                lines.append(f"{'  ' * item_level}[{'x' if checked else ' '}] {item}".rstrip())
            lines.append("")
        elif node.type == "code":
            lines.append("CODE:")
...
def _convert_nodes_to_pdf_reportlab(
    ...
    try:
        ...
        if page_number_alignment not in {"left", "center", "right", "justify"}:
            page_number_alignment = header_align if page_number_position == "header" else footer_align
        page_mode = security.pageNumberMode if security.pageNumberMode in {"page_x", "page_x_of_y"} else "page_x"
        ...
        def draw_aligned_text(text: str, y: float, align: str, *, font_name: str, size: float) -> None:
            content = (text or "").strip()
            if not content:
                return
            c.setFont(font_name, size)
            text_width = pdfmetrics.stringWidth(content, font_name, size)
            if align == "center":
                x = (page_w - text_width) / 2
            elif align == "right":
                x = max(margin_left, page_w - margin_right - text_width)
            else:
                x = margin_left
        ...
        for p_idx, page_lines in enumerate(pages, start=1):
            ...
            if security.headerText:
                header_parts.append(str(security.headerText))
            if page_number_position == "header" and show_header_page_numbers:
                header_parts.append(
                    _resolved_page_number_text(p_idx, len(pages), page_mode, page_number_format)
                )
            if header_parts:
                draw_aligned_text(
                    "   ".join(header_parts),
                    y,
                    page_number_alignment if page_number_position == "header" and show_header_page_numbers else header_align,
            ...
            draw_aligned_text(
                " | ".join(footer_parts),
                margin_bottom * 0.55,
                page_number_alignment if page_number_position == "footer" and show_footer_page_numbers else footer_align,
                font_name="Helvetica",
                size=footer_size,
            )
            c.showPage()
...
class DocumentExporter:
    ...
    def create_preview_html(
        ...
        html = render_preview_html(nodes, css)
        if include_running_blocks:
            return _inject_preview_running_blocks(html, security, theme.styles if isinstance(theme.styles, dict) else {})
        return html
    ...
    def create_docx(
        ...
        for node in nodes:
            if node.type in {"section", "chapter", "appendix", "references_heading"}:
                ...
                _style_paragraph_runs(
                    p,
                    font_name=_font_primary(_style_str(styles, "h1_family", "h1Family", default=font_name)),
                    size_pt=float(getattr(theme.headingStyle, "h1").size or 18),
                    color_hex=getattr(theme.headingStyle, "h1").color or theme.primaryColor,
                    bold=True,
                )
            elif node.type == "toc":
                ...
                _style_paragraph_runs(
                    ...
                    size_pt=float(token.size or max(12, 26 - (level * 2))),
                    color_hex=token.color or theme.primaryColor,
                    bold=str(token.weight or "600") in {"600", "700", "800", "900"},
                )
            elif node.type == "paragraph":
    ...
    def create_export_file(
        ...
        if requested == "pdf":
            ...
            prefer_external = _prefer_ilovepdf_first()
            ok = False
            method = ""
            ...
            if not prefer_external and not ok:
                ok, method = _convert_docx_to_pdf_ilovepdf_api(docx_path, pdf_path)
                attempts.append(method)
            ...
            if not ok and _allow_low_fidelity_pdf_fallback():
            ...
            if ok:
                ...
                if method in {"weasyprint", "reportlab", "emergency_pdf"}:
                    warnings.append(
                        f"PDF generated via fallback renderer ({method}); configure iLovePDF or LibreOffice for closer Word-like fidelity."
                    )
