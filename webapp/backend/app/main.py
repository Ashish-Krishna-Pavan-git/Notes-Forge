...
from .pdf_conversion import PdfConversionService, persist_uploaded_processing_file
from .templates_repo import TemplateRepo


logger = logging.getLogger("notesforge.v10")
...
DEFAULT_PROMPT = (
    "Use NotesForge v10 starter syntax. Prefer H1-H6, BODY/PARAGRAPH/PARA, BULLET, NUMBERED, CODE, TABLE, "
    "IMAGE, FIGURE, DIAGRAM, CAPTION, TOC, LOT, LOF, CHAPTER, REFERENCES, REFERENCE, APPENDIX and PAGEBREAK. "
    "Start each new block with MARKER: payload. After a marker starts a block, keep all following non-marker lines in the "
    "same block until the next marker line appears. This sticky-block rule is especially important for H1-H6, BODY, CAPTION, "
    "REFERENCE, CODE and DIAGRAM. Preserve indentation and tabs exactly inside CODE/DIAGRAM blocks. Use CAPTION immediately "
    "after a TABLE/IMAGE/FIGURE/DIAGRAM when needed. Return only marker lines."
)
...
RUNTIME_DATA_ROOT = Path(os.environ.get("NF_APP_DATA_DIR") or str(BACKEND_ROOT)).resolve()
CANONICAL_CONFIG_PATH = RUNTIME_DATA_ROOT / "config.json"
CANONICAL_THEMES_PATH = RUNTIME_DATA_ROOT / "themes.json"
PROMPT_PATH = RUNTIME_DATA_ROOT / "prompt.txt"
...
def _safe_child_path(base: Path, relative_path: str) -> Path | None:
    try:
...
def _resolve_frontend_dist() -> Path | None:
    explicit = (os.environ.get("NF_FRONTEND_DIST") or "").strip()
...
def _default_theme_record(theme_key: str, name: str, config: Mapping[str, Any], *, builtin: bool) -> Dict[str, Any]:
    ...
    return {
        ...
        "borders": {
            ...
            "shadow": border.get("shadow", False),
            "frame": border.get("frame", False),
        },
        "styles": {},
    }
...
def _normalize_theme_record(theme_key: str, raw: Mapping[str, Any], config: Mapping[str, Any], *, builtin_default: bool) -> Dict[str, Any]:
    ...
    for section in ("fonts", "colors", "spacing", "page", "header", "footer", "heading", "table", "code", "borders", "styles"):
...
def _theme_record_from_config(theme_key: str, name: str, description: str, config: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    record = _deep_merge(base, {section: _as_dict(source.get(section)) for section in ("fonts", "colors", "spacing", "page", "header", "footer")})
    record["name"] = name
...
def _apply_theme_to_config(config: Mapping[str, Any], theme_key: str, theme: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    for section in ("fonts", "colors", "spacing", "page", "header", "footer"):
...
def _theme_to_payload(theme_key: str, theme: Mapping[str, Any]) -> ThemePayload:
    ...
    style_map = {
        ...
        "page_border_shadow": border.get("shadow", False),
        "page_border_frame": border.get("frame", False),
...
class AppState:
    def __init__(self) -> None:
        ...
        self.pdf_conversion = PdfConversionService(self.store)
...
def _parse_detected_batch_items(raw: str | None) -> list[tuple[str, str]]:
    if not raw:
        return []
    try:
        payload = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ValueError(f"Invalid detected_items payload: {exc}")
    if not isinstance(payload, list):
        raise ValueError("detected_items must be a JSON array")
    items: list[tuple[str, str]] = []
    for entry in payload:
        if not isinstance(entry, dict):
            raise ValueError("Each detected_items entry must be an object")
        directory_key = str(entry.get("directoryKey") or entry.get("directory_key") or "").strip()
        filename = str(entry.get("filename") or "").strip()
        if not directory_key or not filename:
            raise ValueError("Each detected_items entry must include directoryKey and filename")
        items.append((directory_key, filename))
    return items


def _run_batch_processing_job(
    state: AppState,
    job_id: str,
    ...
    *,
    ...
    converted_files: list[tuple[str, Path, str, str, str, str, list[str], bool]] = []
    uploaded_inputs: list[Path] = []
    ...
    try:
        total = max(1, len(source_items))
        for index, (source_raw, cleanup_input, output_basename) in enumerate(source_items, start=1):
            source_path = Path(source_raw)
            if cleanup_input:
                uploaded_inputs.append(source_path)
            progress = min(88, 8 + int(((index - 1) / total) * 72))
            with state.lock:
                state.set_job_progress(job_id, status="running", progress=progress)

            result = state.pdf_conversion.convert(
                source_path,
                target_format=target_format,
            ...
            converted_files.append(
                (
                    ...
                    result.external_fallback_used,
                )
            )
            if result.warnings:
                aggregated_warnings.extend([f"{result.output_filename}: {warning}" for warning in result.warnings])
            external_used = external_used or result.external_fallback_used

        file_id, zip_path = state.store.reserve_path("zip")
        report_payload = []
        with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            for (
                ...
                archive.write(output_path, arcname=output_filename)
                report_payload.append(
                    {
                        ...
                        "externalFallbackUsed": external_fallback_used,
                        "warnings": warnings,
                    }
                )
            archive.writestr("batch_report.json", json.dumps(report_payload, indent=2))

        batch_filename = f"notesforge_batch_{target_format}.zip"
        with state.lock:
            state.remember_file_name(file_id, batch_filename)
            token = state.issue_download_token(file_id, batch_filename)
            state.complete_job(
                job_id,
                file_id=file_id,
                ...
                actual_format="zip",
                conversion_engine="batch_processing",
                external_fallback_used=external_used,
                warnings=aggregated_warnings,
                warning=aggregated_warnings[0] if aggregated_warnings else None,
            )
    except Exception as exc:
        with state.lock:
            state.fail_job(job_id, f"Batch processing failed: {exc}")
    finally:
        for input_path in uploaded_inputs:
            try:
...
def create_app() -> FastAPI:
    ...
    async def download(file_id: str):
        ...
        filename = resolved_name or f"notesforge_output.{ext}"
        return FileResponse(path, media_type=media_type, filename=filename)

    @app.get("/api/file-processing/context", response_model=ProcessingContextResponse)
    async def file_processing_context() -> ProcessingContextResponse:
        with state.lock:
            payload = state.file_access.build_context()
        return ProcessingContextResponse(**payload)

    @app.post("/api/file-processing/convert", response_model=ProcessingConversionResponse)
    async def file_processing_convert(
        target_format: str = Form(...),
        provider_preference: str = Form("auto"),
        preserve_layout: bool = Form(True),
        detected_directory_key: str | None = Form(None),
        detected_filename: str | None = Form(None),
        ...
        try:
            if file is not None and (file.filename or "").strip():
                source_path = await persist_uploaded_processing_file(file, state.pdf_conversion.input_dir)
                output_basename = Path(file.filename or source_path.name).stem
                cleanup_input = True
            elif detected_directory_key and detected_filename:
                with state.lock:
                    source_path = state.file_access.resolve_detected_file(detected_directory_key, detected_filename)
                if source_path is None:
                    raise HTTPException(status_code=404, detail="Detected source file was not found")
                output_basename = Path(detected_filename).stem
            else:
                raise HTTPException(
                    status_code=422,
                    detail="Provide an uploaded file or choose a detected file from the runtime directories",
                )
            ...
            with state.lock:
                ...
                state.remember_file_name(result.file_id, result.output_filename)
                state.cleanup_tokens()
                token = state.issue_download_token(result.file_id, result.output_filename)

            return ProcessingConversionResponse(
                success=True,
                downloadUrl=f"/api/download/{token}",
                fileId=result.file_id,
                filename=result.output_filename,
                sourceFormat=result.source_format,  # type: ignore[arg-type]
                targetFormat=result.requested_format,  # type: ignore[arg-type]
                actualFormat=result.actual_format,  # type: ignore[arg-type]
                conversionEngine=result.conversion_engine,
                providerUsed=result.provider_used,
                externalFallbackUsed=result.external_fallback_used,
            ...
            if cleanup_input and source_path is not None:
                try:
                    source_path.unlink(missing_ok=True)
                except OSError:
                    pass

    @app.post("/api/file-processing/convert/batch", response_model=AsyncGenerateResponse, status_code=status.HTTP_202_ACCEPTED)
    async def file_processing_batch_convert(
        target_format: str = Form(...),
        provider_preference: str = Form("auto"),
        preserve_layout: bool = Form(True),
        detected_items: str | None = Form(None),
        files: List[UploadFile] = File(default=[]),
    ) -> AsyncGenerateResponse:
        target = target_format.strip().lower()
        if target not in {"pdf", "docx"}:
            raise HTTPException(status_code=400, detail="target_format must be pdf or docx")

        source_items: list[tuple[str, bool, str]] = []
        upload_dir = state.pdf_conversion.input_dir / uuid4().hex
        ...
        try:
            for upload in files:
                if upload is None or not (upload.filename or "").strip():
                    continue
                persisted = await persist_uploaded_processing_file(upload, upload_dir)
                source_items.append((str(persisted), True, Path(upload.filename).stem))

            for directory_key, filename in _parse_detected_batch_items(detected_items):
                with state.lock:
                    resolved = state.file_access.resolve_detected_file(directory_key, filename)
                if resolved is None:
                    raise HTTPException(status_code=404, detail=f"Detected source file was not found: {filename}")
                source_items.append((str(resolved), False, Path(filename).stem))

            if not source_items:
                raise HTTPException(status_code=422, detail="Provide one or more uploaded or detected files")

            with state.lock:
                job = state.create_job("zip")

            worker = Thread(
                target=_run_batch_processing_job,
                args=(state, job.job_id, source_items),
                kwargs={
            ...
            worker.start()
            return AsyncGenerateResponse(success=True, jobId=job.job_id, status="queued", progress=job.progress)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc))

    @app.get("/api/file-processing/convert/batch/jobs/{job_id}", response_model=GenerateJobStatusResponse)
    async def file_processing_batch_status(job_id: str) -> GenerateJobStatusResponse:
        return await generate_job_status(job_id)

    @app.get("/api/file-processing/convert/batch/jobs/{job_id}/download")
    async def file_processing_batch_download(job_id: str):
        return await generate_job_download(job_id)
    ...
    async def save_prompt(req: PromptUpdateRequest):
        state.prompt_path.parent.mkdir(parents=True, exist_ok=True)
        state.prompt_path.write_text(req.prompt.strip(), encoding="utf-8")
        return {"success": True}

    frontend_dist = _resolve_frontend_dist() if _env_flag("NF_SERVE_FRONTEND", default=False) else None
    if frontend_dist:
        ...
        async def frontend_spa(full_path: str):
            ...
            if not cleaned:
                return FileResponse(frontend_dist / "index.html")
            if cleaned == "api" or cleaned.startswith("api/") or cleaned == "health" or cleaned.startswith("health/"):
                raise HTTPException(status_code=404, detail="Not found")

            asset_path = _safe_child_path(frontend_dist, cleaned)
