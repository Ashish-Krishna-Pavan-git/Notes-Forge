...
def _collect_text_block(lines: Sequence[str], start_idx: int, payload_text: str) -> tuple[str, int]:
    block_lines = [payload_text] if payload_text else []
    next_idx = start_idx + 1
    while next_idx < len(lines):
        if _is_marker_line(lines[next_idx]):
            break
        block_lines.append(lines[next_idx].rstrip())
        next_idx += 1
    return "\n".join(block_lines).rstrip(), next_idx
...
def _collect_caption_entries(nodes: Sequence[Node]) -> tuple[list[str], list[str]]:
    ...
    for node in nodes:
        ...
        if node.type in {"figure", "image"}:
            caption = _single_line_text(node.caption or node.text or "")
        ...
        if node.type == "figure_caption":
            num = _caption_number(state, "figure")
            figures.append(f"Figure {num}: {_single_line_text(node.text)}")
            continue
        if node.type == "table_caption":
            num = _caption_number(state, "table")
            tables.append(f"Table {num}: {_single_line_text(node.text)}")
...
def _collect_toc_entries(nodes: Sequence[Node]) -> list[tuple[int, str]]:
    ...
    for node in nodes:
        if node.type == "heading":
            title = _single_line_text(node.text)
            if title:
        ...
        if node.type == "chapter":
            chapter_idx += 1
            title = _single_line_text(node.text) or "Chapter"
            entries.append((1, f"Chapter {chapter_idx}: {title}"))
            continue
        if node.type == "appendix":
            title = _single_line_text(node.text) or "Appendix"
            entries.append((1, f"Appendix: {title}"))
            continue
        if node.type == "references_heading":
            title = _single_line_text(node.text) or "References"
            entries.append((1, title))
    return entries
...
def _numbered_labels(levels: Sequence[int], count: int) -> list[str]:
    ...
    for idx in range(count):
        ...
        labels.append(".".join(str(value) for value in counters))
    return labels
...
def parse_notesforge(content: str, *, tab_width: int = 4) -> ParseResult:
    ...
    while idx < len(lines):
        ...
        if marker == "H1":
            block_text, next_idx = _collect_text_block(lines, idx, payload_text)
            nodes.append(Node(type="heading", level=1, text=block_text.strip(), marker=raw_marker))
            last_caption_target = ""
            idx = next_idx
            continue
        ...
        if marker == "H2":
            block_text, next_idx = _collect_text_block(lines, idx, payload_text)
            nodes.append(Node(type="heading", level=2, text=block_text.strip(), marker=raw_marker))
            last_caption_target = ""
            idx = next_idx
            continue
        ...
        if marker == "H3":
            block_text, next_idx = _collect_text_block(lines, idx, payload_text)
            nodes.append(Node(type="heading", level=3, text=block_text.strip(), marker=raw_marker))
            last_caption_target = ""
            idx = next_idx
            continue
        ...
        if marker in {"H4", "H5", "H6"}:
            block_text, next_idx = _collect_text_block(lines, idx, payload_text)
            nodes.append(Node(type="heading", level=int(marker[1:]), text=block_text.strip(), marker=raw_marker))
            last_caption_target = ""
            idx = next_idx
            continue
        ...
        if marker in FRONT_MATTER_MARKERS:
            ...
            nodes.append(
                Node(
                    type="section",
                    text=block_text.strip() or FRONT_MATTER_MARKERS[marker],
                    role=marker.lower(),
                    marker=raw_marker,
                )
            )
        ...
        if marker == "CHAPTER":
            ...
            nodes.append(
                Node(
                    type="chapter",
                    text=block_text.strip() or "Chapter",
                    role="chapter",
                    marker=raw_marker,
                )
            )
        ...
        if marker == "APPENDIX":
            ...
            nodes.append(
                Node(
                    type="appendix",
                    text=block_text.strip() or "Appendix",
                    role="appendix",
                    marker=raw_marker,
                )
            )
        ...
        if marker == "REFERENCES":
            ...
            nodes.append(
                Node(
                    ...
                    text=block_text.strip() or "References",
                    role="references",
                    marker=raw_marker,
                )
            )
        ...
        if marker == "REFERENCE":
            block_text, next_idx = _collect_text_block(lines, idx, payload_text)
            nodes.append(Node(type="reference", text=block_text.strip(), role="reference", marker=raw_marker))
            last_caption_target = ""
            idx = next_idx
            continue
        ...
        if marker == "TABLE_CAPTION":
            block_text, next_idx = _collect_text_block(lines, idx, payload_text)
            nodes.append(Node(type="table_caption", text=block_text.strip(), role="table_caption", marker=raw_marker))
            last_caption_target = ""
            idx = next_idx
            continue
        ...
        if marker == "FIGURE_CAPTION":
            block_text, next_idx = _collect_text_block(lines, idx, payload_text)
            nodes.append(Node(type="figure_caption", text=block_text.strip(), role="figure_caption", marker=raw_marker))
            last_caption_target = ""
            idx = next_idx
            continue
        ...
        if marker == "CAPTION":
            ...
            if last_caption_target == "table":
                caption_role = "table_caption"
                caption_type = "table_caption"
            elif last_caption_target not in {"figure", "table"}:
                warnings.append(
                    f"Line {idx + 1}: CAPTION defaults to figure caption; place it after TABLE/IMAGE/FIGURE/DIAGRAM for explicit linking."
                )
            nodes.append(Node(type=caption_type, text=block_text.strip(), role=caption_role, marker=raw_marker))
            last_caption_target = ""
            idx = next_idx
            continue
...
def render_preview_html(
    ...
    for node in nodes:
        if node.type == "heading":
            ...
            fragments.append(f"<h{level}>{_escape_with_breaks(node.text)}</h{level}>")
            ...
            fragments.append(f"<h1>{_escape_with_breaks(title)}</h1>")
        elif node.type == "toc":
            fragments.append(f"<h2>{escape(node.text or 'Table of Contents')}</h2>")
            fragments.append('<ul class="nf-list-root nf-toc-list">')
            for level, item in toc_entries or [(1, "TOC entries appear as headings are added.")]:
                indent_level = max(0, level - 1)
                fragments.append(
                    f'<li class="nf-list-item" style="--nf-level:{indent_level}">{escape(item)}</li>'
                )
            fragments.append("</ul>")
            ...
            items = node.items or []
            levels = node.levels or []
            labels = _numbered_labels(levels, len(items))
            fragments.append('<ol class="nf-list-root nf-numbered-root">')
            for idx, item in enumerate(items):
                item_level = levels[idx] if idx < len(levels) else 0
                item_label = labels[idx] if idx < len(labels) else str(idx + 1)
                fragments.append(
                    '<li class="nf-list-item nf-numbered-item" '
                    f'style="--nf-level:{item_level}"><span class="nf-number-label">{escape(item_label)}.</span>'
...
def to_markdown(nodes: Sequence[Node]) -> str:
    ...
    for node in nodes:
        if node.type == "heading":
            lines.append(f"{'#' * max(1, min(6, node.level))} {_single_line_text(node.text)}")
        elif node.type == "section":
            lines.append(f"# {_single_line_text(node.text)}")
        elif node.type == "chapter":
            caption_state.chapter_idx += 1
            caption_state.figure_chapter = 0
            caption_state.table_chapter = 0
            lines.append(f"# CHAPTER {caption_state.chapter_idx}: {_single_line_text(node.text)}")
        elif node.type == "appendix":
            lines.append(f"# Appendix: {_single_line_text(node.text)}")
            ...
            for idx, item in enumerate(node.items or []):
                item_level = levels[idx] if idx < len(levels) else 0
                item_label = labels[idx] if idx < len(labels) else str(idx + 1)
                lines.append(f"{'  ' * item_level}{item_label}. {item}")
        elif node.type == "checklist":
            levels = node.levels or []
            checks = node.checks or []
            for idx, item in enumerate(node.items or []):
            ...
            text = _single_line_text(node.caption or node.text or "Image")
            ...
            num = _caption_number(caption_state, "table")
            lines.append(f"*Table {num}: {_single_line_text(node.text)}*")
        elif node.type == "figure_caption":
            num = _caption_number(caption_state, "figure")
            lines.append(f"*Figure {num}: {_single_line_text(node.text)}*")
        lines.append("")
    return "\n".join(lines).strip() + "\n"
...
def to_plain_text(nodes: Sequence[Node]) -> str:
    ...
    for node in nodes:
        if node.type == "heading":
            ...
            lines.append("Table of Contents")
            lines.extend(
                f"{'  ' * max(0, level - 1)}- {entry}"
                for level, entry in (toc_entries or [(1, "TOC entries appear as headings are added.")])
            )
        elif node.type == "list_of_tables":
